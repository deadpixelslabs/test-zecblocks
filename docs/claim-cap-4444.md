...
Deployment order: apply `20260924235839_nft_claim_cap_4444.sql` transactionally, deploy `zecblocks-mining-lease` (including `capacity-proof.ts`) and `zecblocks-live-stats`, then publish the tested frontend. Preserve existing Edge authentication settings and service-role secrets. The marketplace update changes displayed supply and documentation, retaining the 5000-ID validation range and current refresh performance.

Deployment record: the tested SQL was applied through the connected Supabase project as `20260924235839_nft_claim_cap_4444`; the file is aligned to that recorded version to prevent a future CLI push from replaying it. Edge releases: mining-lease v12 and live-stats v7. Before/after rollout checks preserve 4,032 protected IDs, including existing ownership above #4444.
